...
const ASTRA_PRIMARY_LINE_ID_START: u64 = 1;
const ASTRA_ALTERNATE_LINE_ID_START: u64 = 1 << 63;

/// Engine-owned identity of one currently reflowed physical row. Wrapped
/// fragments share `logical_line_id`; `cell_offset` is measured in terminal
/// columns from the beginning of that logical line.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct AstraLineIdentity {
...
pub struct Screen {
    ...
    /// Kept in exact lockstep with `lines`. This is deliberately independent
    /// from StableRowIndex, whose meaning changes when resize reflows rows.
...
impl Screen {
    ...
    pub fn new(
        ...
        let mut line_identities = VecDeque::with_capacity(lines.capacity());
        let mut next_logical_line_id = if allow_scrollback {
            ASTRA_PRIMARY_LINE_ID_START
        } else {
            ASTRA_ALTERNATE_LINE_ID_START
        };
        for _ in 0..physical_rows {
            let mut line = Line::new(seqno);
            bidi_mode.apply_to_line(&mut line, seqno);
            lines.push_back(line);
            line_identities.push_back(AstraLineIdentity {
                logical_line_id: next_logical_line_id,
                cell_offset: 0,
            });
            next_logical_line_id = next_logical_line_id
                .checked_add(1)
                .expect("Astra logical line ID exhausted");
        }
    ...
    fn allocate_line_identity(&mut self) -> AstraLineIdentity {
        let identity = AstraLineIdentity {
    ...
    fn assert_astra_identity_invariant(&self) {
    ...
    fn rebase_line_identities(&mut self) {
        let mut identities = VecDeque::with_capacity(self.lines.len());
        for _ in 0..self.lines.len() {
            identities.push_back(self.allocate_line_identity());
        }
        self.line_identities = identities;
        self.astra_identity_epoch = self
            .astra_identity_epoch
            .checked_add(1)
            .expect("Astra line identity epoch exhausted");
    ...
    fn rewrap_lines(
        ...
        let mut rewrapped_identities = VecDeque::new();
        let identities = std::mem::take(&mut self.line_identities);
        let mut logical_line: Option<(Line, AstraLineIdentity)> = None;
        let mut logical_cursor_x: Option<usize> = None;
        let mut adjusted_cursor = (cursor_x, cursor_y);

        for (phys_idx, (mut line, identity)) in
            self.lines.drain(..).zip(identities).enumerate()
        {
            ...
            let (line, identity) = match logical_line.take() {
                None => {
            ...
            if was_wrapped {
                logical_line.replace((line, identity));
                continue;
            }
            ...
            if line.len() <= physical_cols {
                ...
                rewrapped_identities.push_back(identity);
            } else {
                for (fragment_index, line) in line.wrap(physical_cols, seqno).into_iter().enumerate()
                {
                    rewrapped.push_back(line);
                    rewrapped_identities.push_back(AstraLineIdentity {
                        logical_line_id: identity.logical_line_id,
                        cell_offset: identity.cell_offset + fragment_index * physical_cols,
                    });
                }
            }
        }
        self.lines = rewrapped;
        self.line_identities = rewrapped_identities;
        self.assert_astra_identity_invariant();
    ...
    pub fn resize(
        ...
        if capacity > current_capacity {
            ...
            self.line_identities.reserve(capacity - current_capacity);
        }
        ...
        while self.lines.len() < physical_rows {
            ...
            let identity = self.allocate_line_identity();
            self.lines.push_back(Line::new(seqno));
            self.line_identities.push_back(identity);
        }
    ...
    /// Number of currently retained physical rows, including scrollback.
    pub fn astra_row_count(&self) -> usize {
        self.assert_astra_identity_invariant();
        self.lines.len()
    }

    /// Physical row at which the visible viewport begins.
    pub fn astra_viewport_start(&self) -> usize {
    ...
    pub(crate) fn astra_finish_update(&mut self) {
        ...
        for index in 1..self.lines.len() {
            if !self.lines[index - 1].last_cell_was_wrapped()
                && self.line_identities[index - 1].logical_line_id
                    == self.line_identities[index].logical_line_id
            {
    ...
    /// Iterate a caller-bounded contiguous row segment without cloning the
    /// screen. The iterator cannot exceed `maximum_rows`.
    pub fn astra_rows(
        &self,
        start: usize,
        maximum_rows: usize,
    ) -> impl Iterator<Item = (AstraLineIdentity, &Line)> {
        self.assert_astra_identity_invariant();
        self.line_identities
            .iter()
            .copied()
            .zip(self.lines.iter())
            .skip(start)
    ...
    pub fn scroll_up(
        ...
        let (to_remove, to_add) = {
            for _ in 0..to_move {
                ...
                self.line_identities.remove(remove_idx).unwrap();
                ...
                if insert_at_end {
                    ...
                    self.line_identities
                        .insert(phys_scroll.end - 1, identity);
                }
            }
        ...
        for _ in 0..to_remove {
            ...
            self.line_identities.remove(remove_idx);
        }
        ...
        for _ in 0..to_add {
            ...
            if insert_at_end {
                ...
                self.line_identities.insert(phys_scroll.end, identity);
            }
        }
    ...
    pub fn scroll_down(
        ...
        for _ in 0..num_rows {
            ...
            self.line_identities.remove(middle);
        }
        ...
        for _ in 0..num_rows {
            ...
            let identity = self.allocate_line_identity();
            self.line_identities.insert(phys_scroll.start, identity);
        }
        if num_rows > 0 {
            self.rebase_line_identities();
        }
