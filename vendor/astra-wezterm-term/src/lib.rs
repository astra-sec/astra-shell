...
// Upstream's workspace-coupled test fixtures are intentionally not vendored.
// Astra fork behavior is exercised by the root crate's TerminalEngine
// conformance suite against the public read-only Astra view.
